"use client";

import { useState } from "react";
import { motion, AnimatePresence } from "motion/react";
import Link from "next/link";
import {
  Megaphone,
  Search,
  Mail,
  Clapperboard,
  Workflow,
  Users,
  ArrowRight,
} from "lucide-react";
import { Button } from "./button";
import { BOOKING_URL } from "@/lib/site";

type Category = "all" | "paid-media" | "seo" | "retention" | "creative" | "ops";

const CATEGORIES: { id: Category; label: string; icon: React.ReactNode }[] = [
  { id: "all", label: "All Talent", icon: <Users className="size-4" /> },
  { id: "paid-media", label: "Paid Media", icon: <Megaphone className="size-4" /> },
  { id: "seo", label: "SEO & Content", icon: <Search className="size-4" /> },
  { id: "retention", label: "Retention & Email", icon: <Mail className="size-4" /> },
  { id: "creative", label: "Creative", icon: <Clapperboard className="size-4" /> },
  { id: "ops", label: "Ops & Automation", icon: <Workflow className="size-4" /> },
];

type Specialist = {
  name: string;
  role: string;
  years: string;
  stack: string;
  photo: string;
  category: Exclude<Category, "all">;
};

const SPECIALISTS: Specialist[] = [
  {
    name: "Ethan Kowalski",
    role: "Senior Media Buyer",
    years: "8+ Years experience",
    stack: "Meta, Google, TikTok Ads",
    photo: "/talent/talent-dark-1.jpg",
    category: "paid-media",
  },
  {
    name: "Camila Duarte",
    role: "Retention & Email Lead",
    years: "6+ Years experience",
    stack: "Klaviyo, Brevo, HubSpot",
    photo: "/talent/talent-dark-2.jpg",
    category: "retention",
  },
  {
    name: "Diego Salinas",
    role: "Automation Engineer",
    years: "7+ Years experience",
    stack: "GHL, HubSpot, Zapier, n8n",
    photo: "/talent/talent-dark-3.jpg",
    category: "ops",
  },
  {
    name: "Priya Nandakumar",
    role: "Technical SEO Lead",
    years: "9+ Years experience",
    stack: "Ahrefs, GSC, Screaming Frog",
    photo: "/talent/talent-dark-4.jpg",
    category: "seo",
  },
  {
    name: "Marcus Bellweather",
    role: "Google Ads Strategist",
    years: "7+ Years experience",
    stack: "Google Ads, CM360, GA4",
    photo: "/talent/talent-dark-5.jpg",
    category: "paid-media",
  },
  {
    name: "Hana Ishikawa",
    role: "Short-form Video Editor",
    years: "5+ Years experience",
    stack: "Premiere, CapCut, After Effects",
    photo: "/talent/talent-dark-6.jpg",
    category: "creative",
  },
];

export function TalentShowcase() {
  const [active, setActive] = useState<Category>("all");
  const visible =
    active === "all"
      ? SPECIALISTS
      : SPECIALISTS.filter((s) => s.category === active);

  return (
    <section className="relative overflow-hidden bg-[#020409] section-pad">
      <div className="mx-auto max-w-7xl px-4 md:px-8">
        {/* Two-tone display headline, smartworking-style */}
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true, margin: "0px 0px -80px 0px" }}
          transition={{ duration: 0.6, ease: "easeOut" }}
          className="mx-auto max-w-3xl text-center"
        >
          <h2 className="font-display text-4xl font-bold tracking-tight text-brand-accent sm:text-5xl md:text-6xl">
            The <span className="text-white">top 3%</span> looks like this
          </h2>
          <p className="mt-4 text-base text-neutral-400 md:text-lg">
            Fully vetted. Fully managed. Agency-ready.
          </p>
        </motion.div>

        {/* Category tabs */}
        <div className="mt-12 flex flex-wrap items-center justify-center gap-1 border-b border-white/10 pb-px md:gap-2">
          {CATEGORIES.map((cat) => (
            <button
              key={cat.id}
              onClick={() => setActive(cat.id)}
              className={`relative flex cursor-pointer items-center gap-2 px-3 py-3 text-xs font-semibold transition-colors md:px-4 md:text-sm ${
                active === cat.id
                  ? "text-white"
                  : "text-neutral-500 hover:text-neutral-300"
              }`}
            >
              {cat.icon}
              {cat.label}
              {active === cat.id && (
                <motion.span
                  layoutId="talent-tab-underline"
                  className="absolute inset-x-2 -bottom-px h-0.5 rounded-full bg-brand-accent"
                />
              )}
            </button>
          ))}
        </div>

        {/* Rim-lit portrait grid */}
        <div className="mt-4 grid grid-cols-1 gap-x-6 sm:grid-cols-2 lg:grid-cols-4">
          <AnimatePresence mode="popLayout">
            {visible.slice(0, 4).map((s, i) => (
              <motion.article
                key={s.name}
                layout
                initial={{ opacity: 0, y: 24 }}
                animate={{ opacity: 1, y: 0 }}
                exit={{ opacity: 0, y: -12 }}
                transition={{ duration: 0.45, ease: "easeOut", delay: i * 0.08 }}
                className="group pt-8"
              >
                <h3 className="font-display text-xl font-semibold text-brand-accent md:text-2xl">
                  {s.name}
                </h3>
                <div className="relative mt-2 overflow-hidden">
                  <img
                    src={s.photo}
                    alt={`${s.name} — ${s.role}`}
                    className="aspect-[4/4.6] w-full object-cover transition-all duration-500 group-hover:scale-[1.04] group-hover:brightness-125"
                    loading="lazy"
                  />
                  {/* melt the portrait into the section background */}
                  <div className="absolute inset-x-0 bottom-0 h-24 bg-gradient-to-t from-[#020409] to-transparent" />
                </div>
                <ul className="mt-1 space-y-2.5 font-mono text-xs text-neutral-300 md:text-[13px]">
                  <li className="flex items-center gap-2.5">
                    <span className="text-brand-accent">&gt;_</span> {s.role}
                  </li>
                  <li className="flex items-center gap-2.5">
                    <span className="text-brand-accent">&gt;_</span> {s.years}
                  </li>
                  <li className="flex items-center gap-2.5">
                    <span className="text-brand-accent">&gt;_</span> {s.stack}
                  </li>
                </ul>
              </motion.article>
            ))}
          </AnimatePresence>
        </div>

        {/* Long-tail catch-all */}
        <motion.div
          initial={{ opacity: 0, y: 16 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          transition={{ duration: 0.5, ease: "easeOut" }}
          className="mt-16 text-center"
        >
          <p className="font-display text-xl font-bold text-white md:text-2xl">
            Can&apos;t see the exact profile you need?
          </p>
          <p className="mx-auto mt-2 max-w-xl text-sm text-neutral-400 md:text-base">
            Our talent network covers every marketing role — whatever the
            channel, however niche.{" "}
            <Link
              href={BOOKING_URL}
              target="_blank"
              rel="noopener noreferrer"
              className="font-semibold text-brand-accent underline-offset-4 hover:underline"
            >
              Just tell us.
            </Link>
          </p>
          <Link href={BOOKING_URL} target="_blank" rel="noopener noreferrer">
            <Button size="lg" className="mt-8">
              Book a call today <ArrowRight className="size-4" />
            </Button>
          </Link>
        </motion.div>
      </div>
    </section>
  );
}
